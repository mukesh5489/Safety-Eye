...
class PersonDetector:
    def __init__(self, weights: Optional[str] = None, conf: float = 0.05):
        self.conf = max(0.02, conf)
    ...
    def detect(self, frame: np.ndarray) -> List[Box]:
        ...
        out: List[Box] = []
        if self.use_ultra:
            res = self.model(frame, conf=self.conf, imgsz=256, device=self.device, verbose=False)
            for r in res:
...
class PPEUltralytics:
    ...
    def infer(self, frame: np.ndarray) -> List[Box]:
        H, W = frame.shape[:2]
        out: List[Box] = []
        res = self.model(frame, conf=self.conf_th, iou=self.nms_iou, imgsz=256, device=self.device, verbose=False)
        for r in res:
            names = r.names if hasattr(r, 'names') else {}
            for b in r.boxes:
                x1, y1, x2, y2 = map(int, b.xyxy[0].tolist())
...
class Monitor:
    def __init__(self, args: argparse.Namespace):
        ...
        self.skip_frames = max(1, int(getattr(args, 'skip_frames', 2)))
        ...
        self.alert_enabled = bool(args.alert_sender_email and args.alert_sender_pass and args.alert_recipient_email)
        self.alert_smtp_server = args.alert_smtp_server
        self.alert_smtp_port = args.alert_smtp_port
        self.alert_sender_email = args.alert_sender_email
        self.alert_sender_pass = args.alert_sender_pass
        self.alert_recipient_email = args.alert_recipient_email
        self.alerted_tracks: Dict[int, float] = {}
        self.alert_cooldown_s = args.alert_cooldown_s
    ...
    def _send_helmet_alert(self, frame: np.ndarray, person: Box, track_id: int) -> None:
        ...
        success, jpg = cv2.imencode('.jpg', cropped, [int(cv2.IMWRITE_JPEG_QUALITY), 75])
        if not success:
            logger.warning(f"Failed to encode alert image for track {track_id}")
            return

        image_data = jpg.tobytes()
        msg = MIMEMultipart()
        msg['Subject'] = f'Helmet alert: person ID {track_id} missing helmet'
        msg['From'] = self.alert_sender_email
        msg['To'] = self.alert_recipient_email
        body = (
            f"A person without a helmet was detected.\n"
            f"Track ID: {track_id}\n"
            f"Time: {time.strftime('%Y-%m-%d %H:%M:%S')}\n"
            f"Location: {self.args.source}\n"
        )
        msg.attach(MIMEText(body, 'plain'))
        image_part = MIMEImage(image_data, name=f'person_{track_id}.jpg')
        msg.attach(image_part)

        def _send_in_background(msg_bytes: bytes) -> None:
            try:
                context = ssl.create_default_context()
                with smtplib.SMTP_SSL(self.alert_smtp_server, self.alert_smtp_port, context=context) as smtp:
                    smtp.login(self.alert_sender_email, self.alert_sender_pass)
                    smtp.sendmail(self.alert_sender_email, [self.alert_recipient_email], msg_bytes)
                logger.info(f"Sent helmet alert email for track {track_id} to {self.alert_recipient_email}")
            except Exception as e:
                logger.exception(f"Failed to send alert email for track {track_id}: {e}")

        threading.Thread(target=_send_in_background, args=(msg.as_string(),), daemon=True).start()
    ...
    def process_frame(self, frame: np.ndarray) -> np.ndarray:
        ...
        try:
            persons_raw = self.person_det.detect(small_frame)
            persons_raw = persons_raw[:20]
        except Exception as e:
        ...
        ppe_all = self.ppe_backend.infer(small_frame)
        ppe_all = self._filter_boxes(ppe_all)[:25]
        ...
        for i, p in enumerate(persons):
            sh, sv = smoothed[i]
            tid = track_ids[i]
            if not sh and self.alert_enabled:
                last_sent = self.alerted_tracks.get(tid, 0.0)
                if time.time() - last_sent >= self.alert_cooldown_s:
                    self._send_helmet_alert(frame, p, tid)
                    self.alerted_tracks[tid] = time.time()
            elif sh and tid in self.alerted_tracks:
    ...
    # ---------------------------
    def run_stream(self):
        cam1_url = getattr(self.args, 'source', '') or '0'
        logger.info(f"Opening video source: {cam1_url}")
        ...
        cap1 = open_capture(cam1_url)
        if not cap1.isOpened():
            logger.warning('RTSP Camera 1 did not open. Falling back to local camera 0.')
            cap1 = open_capture(0)
            cam1_url = '0'

        if not cap1.isOpened():
            raise RuntimeError(f"Cannot open Camera 1: {cam1_url}")
        ...
        while True:
            ...
            if not ret1 or frame1 is None or frame1.size == 0:
                logger.warning('Reconnecting Camera 1...')
                cap1.release()
                time.sleep(1)
                cap1 = open_capture(cam1_url)
                if not cap1.isOpened() and cam1_url.startswith('rtsp://'):
                    logger.warning('RTSP reconnect failed; trying local camera 0.')
                    cap1 = open_capture(0)
                    cam1_url = '0'
                continue

            try:
                out1 = self.process_frame(frame1)
            except Exception as e:
                logger.error(f'Camera 1 error: {e}')
                out1 = frame1

            cv2.imshow('PPE Monitor', out1)
...
def build_argparser() -> argparse.Namespace:
    ...
    '--source',
    type=str,
    default='rtsp://admin:MngrRWP1@192.168.1.48:554/Streaming/Channels/102',
    ...
    p.add_argument('--person-conf', type=float, default=0.02, help='Confidence for person detector (lower for low-res / RTSP feeds)')
    ...
    p.add_argument('--conf-helmet', type=float, default=0.55)
    p.add_argument('--conf-vest', type=float, default=0.60)
    ...
    p.add_argument('--alert-smtp-server', type=str, default='smtp.gmail.com', help='SMTP server for alert email')
    p.add_argument('--alert-smtp-port', type=int, default=465, help='SMTP SSL port')
    p.add_argument('--alert-sender-email', type=str, default='tanuja9502190765@gmail.com', help='Alert sender email')
    p.add_argument('--alert-sender-pass', type=str, default='nogfvfsbdezyekop', help='SMTP password or app password')
    p.add_argument('--alert-recipient-email', type=str, default='mukesh19222326@gmail.com', help='Alert recipient email')
    p.add_argument('--alert-cooldown-s', type=int, default=30, help='Minimum seconds between repeated alerts for the same track')
    p.add_argument('--eval-root', type=str, default='', help='If set (e.g., dataset/valid), runs evaluation and exits')
    p.add_argument('--skip-frames', type=int, default=2, help='Process every Nth frame to reduce lag on RTSP streams')
    args = p.parse_args()
    if args.ppe_weights == '':
